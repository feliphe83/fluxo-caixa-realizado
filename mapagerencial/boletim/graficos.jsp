    ...
    *{
    ...
    try{
        ...
        rset = stmt.executeQuery("select to_char(ec.datamovimento,'mm') anomes, \n" +
                                 ...
                                 "       sum(decode(s.cod_safra, 72, iec.pesoliquido, 0)) safatu, \n" +
                                 "       sum(decode(s.cod_safra, 66, iec.pesoliquido, 0)) safant \n" +
                                 "FROM agricola.entradacana ec,   \n" +
                                 "     agricola.itensentradacana iec,   \n" +
                                 "     agricola.safra s  \n" +
                                 "WHERE s.cod_grupoempresa   = 1   \n" +
        ...
        rset = stmt.executeQuery("select mes, desmes, proequi_atu, proequi_ant, alcani_atu, alcani_ant, alchid_atu, alchid_ant, alcpro_atu, alcpro_ant, acupro_atu, acupro_ant \n" +
                                 ...
                                 "             sum(case when sf.cod_safra = 72 then decode(Trim(re.nome_variavel),'PROD_EQ',To_Number(nvl(trim(replace(re.valor,',','.')),0)),0) else 0 end) PROEQUI_ATU, \n" +
                                 "             sum(case when sf.cod_safra = 66 then decode(Trim(re.nome_variavel),'PROD_EQ',To_Number(nvl(trim(replace(re.valor,',','.')),0)),0) else 0 end) PROEQUI_ANT, \n" +
                                 "             sum(case when sf.cod_safra = 72 then decode(Trim(re.nome_variavel),'ETANPR',To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCANI_ATU, \n" +
                                 "             sum(case when sf.cod_safra = 66 then decode(Trim(re.nome_variavel),'ETANPR',To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCANI_ANT, \n" +
                                 "             sum(case when sf.cod_safra = 72 then decode(Trim(re.nome_variavel),'ET_PR', To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCHID_ATU, \n" +
                                 "             sum(case when sf.cod_safra = 66 then decode(Trim(re.nome_variavel),'ET_PR', To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCHID_ANT, \n" +
                                 "             sum(case when sf.cod_safra = 72 then decode(Trim(re.nome_variavel),'ETTOPR',To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCPRO_ATU, \n" +
                                 "             sum(case when sf.cod_safra = 66 then decode(Trim(re.nome_variavel),'ETTOPR',To_Number(nvl(trim(re.valor),0)),0) else 0 end) ALCPRO_ANT, \n" +
                                 "             sum(case when sf.cod_safra = 72 and re.codigo_objeto = 23 then decode(Trim(re.nome_variavel),'SAP_VH',To_Number(nvl(trim(replace(re.valor,',','.')),0)),0) else 0 end) ACUPRO_ATU, \n" +
                                 "             sum(case when sf.cod_safra = 66 and re.codigo_objeto = 23 then decode(Trim(re.nome_variavel),'SAP_VH',To_Number(nvl(trim(replace(re.valor,',','.')),0)),0) else 0 end) ACUPRO_ANT \n" +
                                 "      from laboratorio.resultado re, agricola.safra sf \n" +
                                 "      where sf.cod_grupoempresa = 1 \n" +
                                 "      and   sf.cod_empresa      = 1 \n" +
                                 "      and   sf.cod_filial       = 1 \n" +
                                 "      and   sf.cod_safra in (70,72) \n" +
