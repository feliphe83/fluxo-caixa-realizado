...
-- 7) Notas Fiscais sem Entrada (lê a caixa de e-mail de Compras, detecta a
--    chave de acesso das NF-e anexadas em PDF e compara com a entrada no
--    ERP) -> mesma categoria de Suprimentos/Compras do item 4, mesmo fallback.
INSERT INTO intranet_modulo (id_categoria, nome, descricao, icone, url_destino, ordem, ativo)
SELECT c.id,
       'Notas Fiscais sem Entrada',
       'Compara as notas fiscais recebidas por e-mail com a entrada no ERP e alerta as que passarem do prazo',
       'alert',
       '/fluxo-caixa/nf-sem-entrada.html',
       COALESCE((SELECT MAX(m.ordem) + 1 FROM intranet_modulo m WHERE m.id_categoria = c.id), 1),
       1
FROM   intranet_categoria c
WHERE  NOT EXISTS (SELECT 1 FROM intranet_modulo m2 WHERE m2.url_destino = '/fluxo-caixa/nf-sem-entrada.html')
ORDER BY
       CASE
         WHEN UPPER(c.nome) LIKE '%SUPRIMENTO%'   THEN 0
         WHEN UPPER(c.nome) LIKE '%COMPRA%'       THEN 1
...
-- FROM intranet_modulo WHERE url_destino IN ('/fluxo-caixa/pagamento-cana.html','/fluxo-caixa/fechamento-frete.html','/fluxo-caixa/estoque-parado-relatorio.html','/fluxo-caixa/admissao-controle.html','/fluxo-caixa/orcamento-safra.html','/fluxo-caixa/nf-sem-entrada.html');
